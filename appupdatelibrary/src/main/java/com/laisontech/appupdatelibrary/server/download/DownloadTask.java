...
import com.laisontech.appupdatelibrary.okhttp.exception.HttpException;
import com.laisontech.appupdatelibrary.okhttp.exception.OkGoException;
import com.laisontech.appupdatelibrary.okhttp.exception.StorageException;
...
import com.laisontech.appupdatelibrary.server.task.PriorityRunnable;
...
public class DownloadTask implements Runnable {
    ...
    public DownloadTask(String tag, Request<File, ? extends Request> request) {
        ...
        progress.tag = tag;
        progress.folder = OkHttpDownload.getInstance().getFolder();
        progress.url = request.getBaseUrl();
        progress.status = Progress.NONE;
        progress.totalSize = -1;
        progress.request = request;

        executor = OkHttpDownload.getInstance().getThreadPool().getExecutor();
    ...
    public void start() {
        if (OkHttpDownload.getInstance().getTask(progress.tag) == null || DownloadManager.getInstance().get(progress.tag) == null) {
    ...
    public DownloadTask remove(boolean isDeleteFile) {
        ...
        DownloadTask task = OkHttpDownload.getInstance().removeTask(progress.tag);
