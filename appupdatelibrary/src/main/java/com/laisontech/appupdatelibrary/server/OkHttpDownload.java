package com.laisontech.appupdatelibrary.server;
...
import com.laisontech.appupdatelibrary.server.download.DownloadThreadPool;
import com.laisontech.appupdatelibrary.server.task.XExecutor;
...
public class OkHttpDownload {
    ...
    private OkHttpDownload() {
        folder = Environment.getExternalStorageDirectory() + File.separator + "laisonappsupdate" + File.separator;
     ...
     * 从数据库中恢复任务
     */
    public List<DownloadTask> restore(List<Progress> progressList) {
        if (progressList == null || progressList.size() < 1) return null;
        Map<String, DownloadTask> taskMap = OkHttpDownload.getInstance().getTaskMap();
